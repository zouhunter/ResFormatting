...
namespace UFrame.ResFormat
{
    ...
    class AssetAuditorWindow : EditorWindow
    {
        ...
        Rect ruleSelectRect
        {
            get { return new Rect(20f, 10f, position.width - 80, 15); }
        }

         Rect ruleObjRect
        {
            get { return new Rect(ruleSelectRect.max.x, 10f, 20, 15); }
        }
        ...
        Rect EditSelectedPropButtonRect
        {
          ...
          Rect AutoImportPropButtonRect
        {
            get {return new Rect(position.width - 120,70, 120,18 );}
        }
       ...
       //     m_TreeView.Reload();
        ...
        {   
            ...
            if(selected != -1 && selected < assetRules.Count)
            {
                if (GUI.Button(ruleObjRect, "", EditorStyles.objectFieldMiniThumb)){
                    EditorGUIUtility.PingObject( AssetDatabase.LoadAssetAtPath<UnityEngine.Object>(AssetDatabase.GUIDToAssetPath(assetRules[selected].AssetGuid)));
               }
            }
            ...
            if (assetRules != null && selected != -1)// && !string.IsNullOrEmpty(assetRules[selected].WildCard))
            {
                ...
                EditorGUI.BeginChangeCheck();
                ar.WildCard = EditorGUI.TextField(wildCardDisplayRect, "Regular ", ar.WildCard);
                if (EditorGUI.EndChangeCheck())
                {
                ...
                if (ar.SelectiveProperties != null && ar.SelectiveProperties.Count > 0)
                {
                    ...
                    EditorGUI.BeginChangeCheck();
                    ar.autoImport = EditorGUI.ToggleLeft(AutoImportPropButtonRect, "Auto Import", ar.autoImport);
                    if (EditorGUI.EndChangeCheck())
                    {
                        assetRules[selected] = ar;
                        AssetAuditor.WriteUserData(AssetDatabase.GUIDToAssetPath(ar.AssetGuid), ar);
